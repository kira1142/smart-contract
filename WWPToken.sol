...
contract MultiSigWallet {
    ...
    uint256 private immutable _txTimeLive;
    ...
    event SubmitTransaction(address indexed owner, uint8 indexed txType, uint256 indexed txIndex);
    ...
    modifier notConfirmed(uint256 txIndex) {
        require(!_isConfirmed[txIndex][msg.sender], "tx already confirmed");
        _;
    }

    modifier enoughConfirmed(uint256 txIndex) {
        require(
            transactions[txIndex].numConfirmations >= _numConfirmationsRequired,
            "Tx does not have enough confirmations"
        );
        _;
    }

    modifier validTxType(uint256 txIndex, uint8 txType) {
        require(
            transactions[txIndex].txType == txType,
    ...
    modifier notExpired(uint256 txIndex) {
        require(
            transactions[txIndex].expireAt > block.timestamp,
            "Tx was expired"
        );
        _;
    }

    constructor(address[] memory owners_, uint256 numConfirmationsRequired_, uint256 txTimeLive_) {
        require(owners_.length >= 3, "owners require at least 3");
        require(
            numConfirmationsRequired_ >= 2 && numConfirmationsRequired_ <= owners_.length,
            "invalid number of required confirmations"
        );
        require(txTimeLive_ > 0, "invalid time live");
        ...
        notExecuted(txIndex)
    {
        ...
        require(_isConfirmed[txIndex][msg.sender], "tx not confirmed");
    ...
    function isConfirmed(uint256 txIndex, address owner) public view returns (bool) {
        return _isConfirmed[txIndex][owner];
    }

    function _getTransaction(uint256 txIndex) internal view returns (Transaction memory) {
        return transactions[txIndex];
    }

    function _submitTransaction(uint8 _txType, address _to, uint256 _value) internal {
        ...
        transactions.push(
            Transaction({
                ...
                executed: false,
                numConfirmations: 0,
                expireAt: block.timestamp + _txTimeLive
            })
        );

        emit SubmitTransaction(msg.sender, _txType, txIndex);
    }
    ...
    function _transferOwnership(address oldOwner, address newOwner) internal {
        ...
        revert OwnableInvalidOwner(oldOwner);
    }
}

// File: v3/TimeLock.sol
...
contract TimeLock {
    ...
    uint256 immutable private _lockTime;
    ...
    modifier afterUnlock() {
        require(block.timestamp >= _releaseTime, "wait for unlock");
        _;
    }

    function _lock() internal returns (uint256) {
        _releaseTime = block.timestamp + _lockTime;
        return _releaseTime;
    }
...
abstract contract ReleasableERC20 is ERC20, MultiSigWallet, Pausable, ERC20Burnable, TimeLock {

    uint256 private constant TOTAL_SUPPLY = 2_000_000_000 * (10 ** 18); // 2 billion tokens
    uint256 private constant COMMUNITY_SUPPLY = TOTAL_SUPPLY * 85 / 100; // 85%
    uint256 private constant STRATEGIC_RESERVE_SUPPLY = TOTAL_SUPPLY * 5 / 100; // 5%
    uint256 private constant CORE_CONTRIBUTOR_SUPPLY = TOTAL_SUPPLY * 5 / 100; // 5%
    uint256 private constant DEVELOPMENT_TEAM_SUPPLY = TOTAL_SUPPLY * 3 / 100; // 3%
    uint256 private constant ADVISOR_SUPPLY = TOTAL_SUPPLY * 2 / 100; // 2%

    uint256 private immutable START_TIME = block.timestamp;
    uint256 private constant VESTING_MONTHS = 36;
    uint256 private constant SECONDS_PER_MONTH = 30 days;

    address private immutable _communityWallet;
    address private immutable _strategicReserveWallet;
    address private immutable _coreContributorsWallet;
    address private immutable _developmentTeamWallet;
    address private immutable _advisorsWallet;
    ...
    enum TransactionType {PAUSE, UNPAUSE, TRANSFER_OWNERSHIP, WITHDRAW}

    event Withdraw(address indexed to, uint256 amount);
    event TokensReleased(address indexed beneficiary, uint256 amount);
    event SubmitPause(address indexed sender);
    event SubmitUnpause(address indexed sender);
    event SubmitWithdraw(address indexed to, uint256 amount);
    event SubmitTransferOwnership(address indexed oldOwner, address indexed newOwner);
    event Lock(uint256 indexed txIndex, uint256 releaseTime);
    ...
    ) ERC20(name_, symbol_) MultiSigWallet(owners_, numConfirmationsRequired_, txTimeLive_) TimeLock(lockTime_) {
    ...
    function getReleasableAmount(address beneficiary, uint256 timestamp) public view returns (uint256) {
        ...
        uint256 elapsedMonths = Math.min((timestamp - START_TIME) / SECONDS_PER_MONTH, VESTING_MONTHS - 1);
        uint256 releasableAmount = 0;

        for (uint256 i = 0; i <= elapsedMonths; i++) {
            releasableAmount += (totalAllocation * releaseSchedule[i] / 10000);
        }

        if (releasableAmount > _releasedTokens[beneficiary]) {
            return releasableAmount - _releasedTokens[beneficiary];
        }
        return 0;
    }
    ...
    function submitPause() whenNotPaused onlyOwner external {
        _submitTransaction(uint8(TransactionType.PAUSE), address(0), 0);
        emit SubmitPause(msg.sender);
    }

    function submitUnpause() whenPaused onlyOwner external {
        _submitTransaction(uint8(TransactionType.UNPAUSE), address(0), 0);
        emit SubmitUnpause(msg.sender);
    }

    function submitWithdraw(uint256 amount) onlyOwner external {
        require(amount > 0 && amount <= address(this).balance, "Invalid withdraw amount");
        _submitTransaction(uint8(TransactionType.WITHDRAW), msg.sender, amount);
        emit SubmitWithdraw(msg.sender, amount);
    }

    function submitTransferOwnership(address newOwner) onlyOwner external {
        require(!this.isOwner(newOwner), "Invalid newOwner");
        _submitTransaction(uint8(TransactionType.TRANSFER_OWNERSHIP), newOwner, 0);
        emit SubmitTransferOwnership(msg.sender, newOwner);
    }

    // Withdraw function to withdraw Bnb from the contract
    function withdraw(uint256 txIndex)
        external
        onlyOwner
        txExists(txIndex)
        notExpired(txIndex)
        notExecuted(txIndex)
        validTxType(txIndex, uint8(TransactionType.WITHDRAW))
        enoughConfirmed(txIndex)
        afterUnlock
    {
        Transaction memory transaction = _getTransaction(txIndex);
        ...
        _postExecuteTransaction(txIndex);
    }

    // Only new owner can call
    function approveTransferOwnership(uint256 txIndex)
        external
        txExists(txIndex)
        notExpired(txIndex)
        notExecuted(txIndex)
        validTxType(txIndex, uint8(TransactionType.TRANSFER_OWNERSHIP))
        enoughConfirmed(txIndex)
        afterUnlock
    {
        Transaction memory transaction = _getTransaction(txIndex);
        require(!this.isOwner(msg.sender), "sender must not be owner");
        require(msg.sender == transaction.to, "not allow");
        require(this.isOwner(transaction.from), "invalid old owner");

        _transferOwnership(transaction.from, transaction.to);
        _postExecuteTransaction(txIndex);
    }

    function pause(uint256 txIndex)
        external
        onlyOwner
        txExists(txIndex)
        notExpired(txIndex)
        notExecuted(txIndex)
        validTxType(txIndex, uint8(TransactionType.PAUSE))
        enoughConfirmed(txIndex)
        afterUnlock
    {
        _pause();
    ...
    function unpause(uint256 txIndex)
        external
        onlyOwner
        txExists(txIndex)
        notExpired(txIndex)
        notExecuted(txIndex)
        validTxType(txIndex, uint8(TransactionType.UNPAUSE))
        enoughConfirmed(txIndex)
        afterUnlock
    {
        _unpause();
    ...
    function _releaseTokens(address beneficiary, uint256 time) internal {
        ...
        _releasedTokens[beneficiary] += unreleased;
    ...
    function _postExecuteTransaction(uint256 txIndex) private {
        _executeTransaction(txIndex);
        uint256 releaseTime = _lock();
        emit Lock(txIndex, releaseTime);
    }
}

// File: v3/WWPToken.sol


pragma solidity ^0.8.20;


contract WWPToken is ReleasableERC20 {
    constructor(
        address[] memory owners_,
        uint256 numConfirmationsRequired_,
        uint256 txTimeLive_,
        ...
        ReleasableERC20(
            owners_,
            numConfirmationsRequired_,
            txTimeLive_,
            lockTime_,
            communityWallet_,
            strategicReserveWallet_,
            coreContributorWallet_,
            developmentTeamWallet_,
            advisorWallet_,
            name_,
            symbol_
        )
